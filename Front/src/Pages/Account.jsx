    ...
    <div style={{ padding: '20px' }}>
      <Box sx={{ background: Colors.light_gray, padding: '10px', borderRadius: '10px', }}>
        <Box display={"flex"} justifyContent={"space-between"} alignItems={"center"} flexDirection={isMobile? "column":"row"}>
          ...
          <Box justifyContent={decodedToken.admin || isMobile ? "space-evenly": "flex-end"} sx={{ display: "flex", p: 1, width: "360px" }}>
            {decodedToken.admin ? <Button variant="contained" color="error" sx={{ color: "white", fontWeight: 600 }} href='/admin'>
              Panel Admin
            </Button> : <></>}

            <Button variant="contained" color="error" sx={{ color: "white", fontWeight: 600 }} onClick={handleLogout}>
              Cerrar sesión
            </Button>
          </Box>
        </Box>
        ...
        <Box marginBottom={2}>
          {decodedToken.admin ? <Typography sx={{ marginTop: "28px", marginBottom: "28px" }} variant="h6" gutterBottom>
              ...
              <Grid item key={purchase.id} xs={12} sm={6} md={4} lg={3}>
                ...
                >
                  ...
                  <Typography variant="body2" sx={{ color: 'black' }} gutterBottom>
                    Estado: {purchase.estado === 'PAGADO CON EXITO'? "PAGADO CON ÉXITO": "NO PAGADO"}
                  </Typography>
        ...
        <Box marginBottom={2}>
          {decodedToken.admin ? <Typography variant="h6" gutterBottom>
            Cantidad de Compras: {totalPurchases}
          </Typography> : <></>}
        </Box>

        {decodedToken.admin ? <> </> :<Box sx={{ display: 'flex', justifyContent: 'center' }}>
          <Box
            marginBottom={2}
            sx={{
              display: 'flex',
